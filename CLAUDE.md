...
- **Perceived-latency detail**: `EditorDialog.vue`'s `v-dialog` is `eager`, and its draft-seeding watcher fires on `src` changing (not on `modelValue`) — so the crop tool mounts and `src/utils/preload.ts#preloadImage` finishes decoding *before* `editorOpen` flips true. This is intentional: don't move the seeding watcher back to `modelValue` or the open transition will show a blank/loading crop tool again.
